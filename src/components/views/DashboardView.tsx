  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <KPICard title="Pacientes Cadastrados" value={patientCount.toLocaleString('pt-BR')} subtitle="Total no sistema" icon={Users} loading={loading} />
        <KPICard title="Consultas Hoje" value={todayAppointments.toString()} subtitle="Agendadas para hoje" icon={Calendar} loading={loading} />
        <KPICard title="Faturamento Hoje" value={formatCurrency(todayRevenue)} subtitle="Receita do dia" icon={DollarSign} trend={5} loading={loading} />
        <KPICard
      ...
      <div className="grid gap-4 lg:grid-cols-3">
            ...
            {loadingChart ? <Skeleton className="h-48 w-full" /> : (
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={revenueChart}>
                  ...
                  <YAxis tickFormatter={(v) => `Kz${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 12 }} />
        ...
        <Card>
          ...
          <CardContent>
            {loadingList ? <Skeleton className="h-48 w-full" /> : statusData.length === 0 ? (
